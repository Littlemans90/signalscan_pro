...
class Tier1Alpaca:
    ...
    def calculate_avg_volumes(self, symbols):
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            try:
                ...
                self.log.scanner(f"[TIER1-ALPACA-DEBUG] Batch {batch_num} - bars type: {type(bars)}, data: {bars}")
    ...
    def filter_tickers(self, symbols):
        """
        Filter tickers by:
        ...
        Batches into groups of 60 symbols with 0.5 second delay between batches
        """
        filtered = []
        total = len(symbols)
        batch_size = 75
        ...
        # Calculate 30-day average volumes for all symbols first
    ...
    def run_scan(self):
        ...
        # Filter symbols
        filtered = self.filter_tickers(symbols)
        
        # Save to prefilter.json
        self.file_manager.save_prefilter(filtered)
        self.log.scanner(f"[TIER1-ALPACA] OK Saved {len(filtered)} symbols to prefilter.json")
